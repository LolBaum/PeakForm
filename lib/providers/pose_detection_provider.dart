...
class PoseDetectionProvider extends ChangeNotifier {
  ...
  static const int inputSize = 192; // SinglePose Lightning input size: [1, 192, 192, 3]
  static const int numKeypoints = 17;
  static const double confidenceThreshold = 0.01; // Very low threshold to see all detections
  ...
  Future<void> _loadModel() async {
    try {
      ...
      if (inputShape[1] != inputSize || inputShape[2] != inputSize) {
        throw Exception('Invalid model input size. Expected ${inputSize}x$inputSize, got ${inputShape[1]}x${inputShape[2]}');
      }
      
      // SinglePose output: [1, 1, 17, 3] = 1 person, 17 keypoints, 3 values per keypoint
      if (outputShape.length < 4 || outputShape[2] != numKeypoints || outputShape[3] != 3) {
        debugPrint('⚠️ Unexpected output shape: $outputShape - expected [1, 1, 17, 3]');
      }
  ...
  void startDetection() {
    ...
    if (!isPlatformSupported) {
      _detectionStatus = 'Pose detection not supported on web. Use Android/iOS device.';
      notifyListeners();
      return;
    }
    ...
    _frameCount = 0;
    _detectionStatus = 'MoveNet SinglePose Lightning detecting pose in real-time...';
  ...
  void _processCameraImage(CameraImage image) async {
    ...
    try {
      ...
      if (_frameCount % 100 == 0) {
        ...
        debugPrint('📊 Input tensor stats: min=$minVal, max=$maxVal, avg=${avgVal.toStringAsFixed(3)}');
      }
      ...
      if (_frameCount % 20 == 0) {
        ...
        final totalKeypoints = poses.isNotEmpty 
            ? poses.first.landmarks.where((l) => l.confidence > confidenceThreshold).length 
            : 0;
        _detectionStatus = 'SinglePose Lightning: ${fps.toStringAsFixed(1)} FPS | ${hasDetection ? "Person detected" : "No person"}, $totalKeypoints keypoints';
        
        // Reduced debug output frequency
        if (_frameCount % 100 == 0) {
          debugPrint('🎯 SinglePose frame $_frameCount: ${fps.toStringAsFixed(1)} FPS, ${hasDetection ? "detected" : "no detection"}');
        }
      }
  ...
  List<Pose> _parseModelOutput(List<List<List<List<double>>>> outputData) {
    ...
    if (_frameCount % 50 == 0) {
      ...
      debugPrint('  Visible landmarks (>${confidenceThreshold.toStringAsFixed(2)}): ${visibleLandmarks.length}');
      debugPrint('  Pose confidence: ${poseConfidence.toStringAsFixed(3)}');
      if (visibleLandmarks.isNotEmpty) {
        debugPrint('  Max confidence: ${landmarks.map((l) => l.confidence).reduce(math.max).toStringAsFixed(3)}');
        debugPrint('  Min confidence: ${landmarks.map((l) => l.confidence).reduce(math.min).toStringAsFixed(3)}');
      }
    }
    
    // Only return pose if we have sufficient keypoints
    if (visibleLandmarks.length >= 1) { // Only need 1 keypoint for debugging
